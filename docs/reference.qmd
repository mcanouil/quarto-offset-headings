...
| `max-level` | integer | `6` | Deepest level a heading that writes an offset, or inherits one from a cascade, may be pushed to. |
...
The cap applies to a heading whose `offset-headings-by` the schema accepts, and to a heading that inherits a cascade.
A heading whose value the schema rejects escapes the cap as well.
A heading that does neither takes the document offset and is clamped to the range one to six only, whatever `max-level` says.
...
| `max-level` is between one and six. | Reported as an error, and the value is clamped. |
| `by`, `max-level`, and `depth` are integers. | Reported as an error, and the option falls back to its default. |
...
A fractional number is rejected and reported, and the heading takes the document offset alone.
It neither starts a cascade nor inherits one, so it is not the same as leaving the attribute out.
